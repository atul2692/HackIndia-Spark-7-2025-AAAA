...
// Add type definition for Google
declare global {
  interface Window {
    google?: {
      accounts: {
  ...
  return (
    <header className={`py-4 px-4 md:px-8 lg:px-12 fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="container mx-auto flex justify-between items-center">
        ...
        >
          <Link href="/signin" className="relative px-4 py-2 text-white font-medium group transition-colors">
...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    if (error === 'google_auth_failed') {
      ...
      setLocalError('OAuth client not found. Please contact support for assistance.');
    } else if (error === 'invalid_grant') {
      setLocalError('Authentication grant invalid or expired. Please try signing in again.');
    } else if (error === 'missing_code') {
      setLocalError('Authentication code missing. Please try signing in again.');
    }
  ...
  useEffect(() => {
    if (googleScriptLoaded && window.google) {
      window.google.accounts.id.initialize({
        client_id: '642417892134-sjvcnakr2q999nu5ldkqm2ve0dad55og.apps.googleusercontent.com', 
  ...
  const handleGoogleSignIn = (response: any) => {
    if (response.credential) {
      // Use the backend endpoint with the ID token
      window.location.href = `http://localhost:8000/accounts/google/login/?id_token=${response.credential}`;
    }
  };

  // Add direct sign-in method similar to sign-up page
              ...
              <motion.div 
                ...
                animate={{ 
                  background: [
                    ...
                    "radial-gradient(circle at 40% 30%, rgba(99, 102, 241, 0.7) 0%, rgba(17, 24, 39, 0) 50%)",
                    "radial-gradient(circle at 60% 60%, rgba(76, 29, 149, 0.7) 0%, rgba(17, 24, 39, 0) 50%)",
                  ]
                }}
                transition={{ 
                  duration: 15, 
              ...
              <div className="text-center mb-8">
                ...
                >
                  <h1 className="text-3xl font-bold text-white mb-2">
                    <span className="text-indigo-400">Welcome</span> Back
                  </h1>
                  <p className="mt-2 text-gray-400 font-light">
                    <AnimatedText text="Sign in to your FairSight AI account" />
                  </p>
                </motion.div>
              </div>
                ...
                >
                  <div className="flex items-center">
                    <input
                      ...
                      name="remember-me"
                      type="checkbox"
                      className="h-4 w-4 bg-gray-800 border-gray-700 rounded text-indigo-600 focus:ring-indigo-500"
                    />
              ...
              >
                ...
                <div className="mt-6 flex justify-center">
                  ...
                  <motion.button
                    ...
                    className="flex items-center justify-center w-full py-3 px-4 rounded-lg border border-gray-700 bg-gray-800/70 hover:bg-gray-800 text-gray-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200"
                    variants={buttonVariants}
                    initial="initial"
                    whileHover="hover"
                    whileTap="tap"
